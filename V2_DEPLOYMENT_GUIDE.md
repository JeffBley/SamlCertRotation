# SAML Certificate Rotation Tool - V2 Deployment Guide (PowerShell)

This guide walks you through deploying the **v2 branch** of the SAML Certificate Rotation Tool using **PowerShell**. This deployment uses unique resource names so it can run side-by-side with a main-branch deployment. All commands are written for **Azure Cloud Shell** (recommended) but work in any local environment with the prerequisites installed.
...
- Azure Subscription with **Owner** access (Contributor is insufficient ΓÇö RBAC role assignments are required)
...
@"
# Saved by Step 1.6 ΓÇö sourced automatically by later steps
...
2. Navigate to **Protection** ΓåÆ **Custom security attributes**
...
# Load deployment outputs ΓÇö try local file first, fall back to Azure
$outputs = $null
...
You should see **29 functions** listed. The route check should return `401` (authentication required). A `404` indicates the `.azurefunctions` directory is missing from the zip ΓÇö see the note in Step 5.2.

> **Note:** On Consumption plans, `az functionapp function list` may occasionally return an empty list even when functions are deployed correctly. If the list is empty but the route check returns `401` (not `404`), your functions are indexed and working. You can also verify via the Azure Portal under **Function App ΓåÆ Functions**.
...
$appRoles = @(
    ...
    @{
        ...
        description = "Sponsor access ΓÇö view and manage own sponsored apps"
...
2. Navigate to **Applications** ΓåÆ **Enterprise applications**
3. Open your enterprise app (`SAML Certificate Rotation Dashboard v2`)
4. Go to **Users and groups** ΓåÆ **Add user/group**
...
4. Go to **Properties** ΓåÆ **Custom security attributes**
...
1. Go to [Microsoft Entra admin center](https://entra.microsoft.com) ΓåÆ **Applications** ΓåÆ **App registrations**
2. Select **SAML Certificate Rotation Dashboard v2**
...
1. Navigate to Azure Portal ΓåÆ Static Web App ΓåÆ **Settings** ΓåÆ **Custom domains** 
2. Click **Add**.
3. Select **Custom domain on other DNS**
4. Enter your domain name (e.g., `saml.domain.com`) and click **Next**.
5. Set **Hostname record type** to **CNAME**.
6. Note the values and click **add**.
7. In your DNS provider, add a **CNAME** record:
   - **Name/Host**: `saml` (or the subdomain portion of your domain)
   - **Value**: the auto-generated dashboard URL (e.g., `happy-island-01f529a0f.azurestaticapps.net`)
DNS propagation may take several (5-15+) minutes.

**NOTE:** By verifying with a CNAME record, Azure will automatically generate the SSL certificate and DNS resolution will work properly. If you verify your domain via TXT record you will still have to add the CNAME record.

#### 2. Update the App Registration redirect URI

1. Go to [Microsoft Entra admin center](https://entra.microsoft.com) ΓåÆ **App registrations** ΓåÆ `SAML Certificate Rotation Dashboard v2`
2. Click **Authentication**
3. Click **Add Redirect URIs**
4. Select **Web** 
5. Add `https://saml-dashboard.yourcompany.com/.auth/login/aad/callback` and click **Configure**

You may keep the original `azurestaticapps.net` redirect URI as a fallback or delete it.
...
The `SWA_HOSTNAME` setting is read by [DashboardFunctionBase.cs](src/SamlCertRotation/Functions/DashboardFunctionBase.cs) to build a trusted issuer (`https://saml-dashboard.yourcompany.com/.auth`). No code changes are needed ΓÇö the setting is already supported.
...
1. Go to your **Function App** ΓåÆ **Settings** ΓåÆ **Environment variables**
2. Add or update the `RotationSchedule` setting with a CRON expression:
   - `0 0 6 * * *` ΓÇö Daily at 6:00 AM UTC (default)
   - `0 0 */12 * * *` ΓÇö Every 12 hours
   - `0 0 6 * * 1` ΓÇö Every Monday at 6:00 AM UTC
...
2. Add or update the `StaleCertCleanupSchedule` setting:
   - `0 0 6 1 * *` ΓÇö 1st of every month at 6:00 AM UTC (default)
   - `0 0 6 15 * *` ΓÇö 15th of every month at 6:00 AM UTC
   - `0 0 6 * * 1` ΓÇö Every Monday at 6:00 AM UTC
   - `0 0 6 1 */3 *` ΓÇö Quarterly (1st of every 3rd month)
3. **Restart the Function App** for changes to take effect
...
`AAD_CLIENT_SECRET` should be an `@Microsoft.KeyVault(...)` reference string ΓÇö not a plaintext secret value. <br > <br>
Optional #6
```
