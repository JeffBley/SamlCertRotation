...
public class DashboardFunctions
{
    ...
    /// <summary>
    /// Temporary endpoint to inspect auth headers and token claims from SWA.
        ...
        [HttpTrigger(AuthorizationLevel.Anonymous, "get", Route = "debug/identity")] HttpRequestData req)
    {
        ...
        var clientPrincipalHeader = GetHeaderValue(req, "x-ms-client-principal");
        var authTokenHeader = GetHeaderValue(req, "x-ms-auth-token");
        var authToken = authTokenHeader?.StartsWith("Bearer ", StringComparison.OrdinalIgnoreCase) == true
            ? authTokenHeader.Substring("Bearer ".Length).Trim()
            : authTokenHeader;

        object? jwtPayloadSummary = null;
        object? prnSummary = null;

        var jwtPayloadJson = DecodeJwtPayload(authToken ?? string.Empty);
        if (!string.IsNullOrWhiteSpace(jwtPayloadJson))
        {
            try
            {
                ...
                jwtPayloadSummary = new
                {
                    oid = TryGetString(payloadRoot, "oid"),
                    sub = TryGetString(payloadRoot, "sub"),
                    roles = ReadClaimValues(payloadRoot, "roles"),
                    role = ReadClaimValues(payloadRoot, "role"),
                    roleUri = ReadClaimValues(payloadRoot, "http://schemas.microsoft.com/ws/2008/06/identity/claims/role"),
                    groups = ReadClaimValues(payloadRoot, "groups"),
                    groupUri = ReadClaimValues(payloadRoot, "http://schemas.microsoft.com/ws/2008/06/identity/claims/groups"),
                    prnPresent = payloadRoot.TryGetProperty("prn", out var _)
                };

                if (payloadRoot.TryGetProperty("prn", out var prnElement) && prnElement.ValueKind == JsonValueKind.String)
                {
                    var prnRaw = prnElement.GetString() ?? string.Empty;
                    var prnDecoded = DecodePrincipalPayload(prnRaw) ?? Uri.UnescapeDataString(prnRaw);
                    if (!string.IsNullOrWhiteSpace(prnDecoded))
                    {
                        using var prnDocument = JsonDocument.Parse(prnDecoded);
                        var principalRoot = prnDocument.RootElement;
                        ...
                        var principalIdentity = BuildIdentityFromPrincipalRoot(principalRoot);

                        prnSummary = new
                        {
                            userId = TryGetString(principalRoot, "userId"),
                            userRoles = ReadClaimValues(principalRoot, "userRoles"),
                            rolesClaims = ReadClaimsByType(principalRoot, new[]
                            {
                                "roles",
                                "http://schemas.microsoft.com/ws/2008/06/identity/claims/role"
                            }),
                            groupClaims = ReadClaimsByType(principalRoot, new[]
                            {
                                "groups",
                                "http://schemas.microsoft.com/ws/2008/06/identity/claims/groups"
                            }),
                            mappedIdentity = principalIdentity == null
                                ? null
                                : new
                                {
                                    principalIdentity.UserId,
                                    principalIdentity.IsAuthenticated,
                                    Roles = principalIdentity.Roles.OrderBy(r => r).ToArray()
                                }
                        };
                    }
                }
        ...
        var response = new
        {
            message = "Temporary debug endpoint. Remove after auth troubleshooting.",
            hasClientPrincipalHeader = !string.IsNullOrWhiteSpace(clientPrincipalHeader),
            hasAuthTokenHeader = !string.IsNullOrWhiteSpace(authTokenHeader),
            clientPrincipalHeaderPreview = TruncateForDebug(clientPrincipalHeader),
            authTokenHeaderPreview = TruncateForDebug(authTokenHeader),
            ...
            authConfig = new
            {
                adminRole = _configuration["SWA_ADMIN_APP_ROLE"],
                readerRole = _configuration["SWA_READER_APP_ROLE"],
                adminGroup = _configuration["SWA_ADMIN_GROUP_ID"],
                readerGroup = _configuration["SWA_READER_GROUP_ID"]
            },
            headers = req.Headers
                ...
                .ToDictionary(
                    h => h.Key,
                    h => h.Value?.Select(TruncateForDebug).ToArray() ?? Array.Empty<string>(),
    ...
    private static string[] ReadClaimValues(JsonElement root, string propertyName)
    {
    ...
    private static string[] ReadClaimsByType(JsonElement principalRoot, IEnumerable<string> claimTypes)
    {
        if (!principalRoot.TryGetProperty("claims", out var claimsElement) || claimsElement.ValueKind != JsonValueKind.Array)
        {
            return Array.Empty<string>();
        }
