...
if (-not $outputs) {
    ...
    if (-not $latestDeploymentName) {
        throw "No successful deployment found in resource group '$RESOURCE_GROUP'. Run Step 3.2 first."
    }
...
3. Open the app registration created in Step 6.1 (`SAML Certificate Rotation Dashboard`)
...
# Update staticwebapp.config.json with tenant ID from Step 6.9
...
By default, your dashboard URL is auto-generated (e.g., `happy-island-01f529a0f.azurestaticapps.net`). To use a custom domain like `saml-dashboard.yourcompany.com`, you need to update **four** places:
...
Or via the Azure Portal: Static Web App → **Custom domains** → **Add** → follow the DNS validation steps. Azure provides a free SSL certificate automatically.
...
3. Under **Redirect URIs**, add: `https://saml-dashboard.yourcompany.com/.auth/login/aad/callback`
4. Keep the original `azurestaticapps.net` redirect URI as a fallback

#### 3. Add the `SWA_HOSTNAME` Function App setting

The Function App must trust the custom domain as an additional SWA token issuer:

```powershell
az functionapp config appsettings set `
    --resource-group $RESOURCE_GROUP `
    --name $FUNCTION_APP_NAME `
    --settings "SWA_HOSTNAME=saml-dashboard.yourcompany.com"
```

This setting is read by [DashboardFunctions.cs](src/SamlCertRotation/Functions/DashboardFunctions.cs) to build a trusted issuer (`https://saml-dashboard.yourcompany.com/.auth`). No code changes are needed — the setting is already supported.

#### 4. Update CORS on the Function App

Add the custom domain to the Function App's CORS allowed origins:
...
az functionapp cors add `
    --resource-group $RESOURCE_GROUP `
    --name $FUNCTION_APP_NAME `
    --allowed-origins "https://saml-dashboard.yourcompany.com"
```
